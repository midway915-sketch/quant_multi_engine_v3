...
def build_branch5a_holdings(
    ...
    return equity_series, holdings_df, rebalance_df
...
def run_one(
    ...
    equity, holdings_df, rebalance_df = build_branch5a_holdings(
        prices=prices,
        lookback=lookback,
        rebalance=rebalance,
    ...
    return equity, holdings_df, rebalance_df, summary_row
...
def main() -> None:
    ...
    parser.add_argument("--lookback", type=int, default=133)
    parser.add_argument("--rebalance", type=str, default="weekly")
    parser.add_argument("--top1-weight", type=float, default=0.80)
    ...
    holdings_df.to_csv(out_dir / "holdings_daily.csv", index=False)
    ...
    metrics_json = {
        ...
        "top2_weight": float(1.0 - args.top1_weight),
        "buy_cost": float(args.buy_cost),
        "sell_cost": float(args.sell_cost),
        "full": {
            "cagr": float(summary_row["cagr"]),
            "mdd": float(summary_row["mdd"]),
            "max_recovery_days": int(summary_row["max_recovery_days"]),
            "seed_multiple": float(summary_row["seed_multiple"]),
        },
        "recent_10y": {
            "cagr": float(summary_row["cagr_10y"]),
            "mdd": float(summary_row["mdd_10y"]),
            "max_recovery_days": int(summary_row["max_recovery_10y_days"]),
            "seed_multiple": float(summary_row["seed_multiple_10y"]),
        },
    }
